...
{{
  "Tactic_Name": "string - 战术名称，简洁专业（4-8字），如 交替掩护撤退、切片侦察突入。禁止 型/式/法/模式 后缀",
...
以下规则在概念生成阶段即适用——战术概念是通用战术模式的雏形，不是场景脚本：

1. **禁止绝对方向**: 描述中不得出现 东/南/西/北 + 侧/端/段/翼/面/墙/角。用关系描述（远端/近端/一侧/对侧）替代
2. **禁止具体物体**: 不得出现桌子/沙发/椅子/木质/金属/混凝土。用战术功能类别（低矮掩体/大型遮挡物/柱状掩体）替代
3. **禁止具体数值**: Description 中不得出现阿拉伯数字（距离/时间/数量）
4. **使用编组名**: 执行主体为"突击组""侦察组""掩护组"等功能编组，禁止机器人编号
5. **纯中文**: 战术文本全部为中文，不含英文词汇
