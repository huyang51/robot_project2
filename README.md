...
| **场景无关** | 战术描述行动**模式**，不绑定特定实例，禁止具体数值与品牌名，禁止绝对方向指代（东/南/西/北+侧/端/段） |
...
```
  ...
  │         • A_review: 15 条通用性规则 (含 G-T10 绝对方向禁止) + 军事可行性语义审查   │
...
desc.json + scene_cubes.json
        ...
        ▼
   ┌──────────────────┐
   │ 方向泛化预处理     │  desc.json 绝对方向 → 功能/关系描述
   │ direction_generalizer│  从源头切断方向信息污染
   └────────┬─────────┘
        │
        ▼
   ┌─────────┐
...
### M3 通用性审查规则 (15 条，全部由 A_review LLM 语义审查)

**文字描述版 (G-T1~G-T10)**：检查 text_version 的 Description / Objective / Action_Sequence Intent / Visual_Aids
...
| G-T10 | 硬 | 禁止绝对方向指代 | 禁止东/南/西/北+侧/端/段/翼/方向（如"走廊西端"、"北侧开口"）；必须替换为功能/关系描述（如"走廊远端"、"走廊一侧的开口"）；方向作为角色职能时除外（如"侧翼警戒手"） |
...
> **设计决策**：全部 15 条规则由 A_review LLM 做语义审查。此前尝试用 Python 正则做前置预检，但中文文本的语义判断（区分步骤编号与参数绑定、区分方向用字与数量用字、区分绝对方向词与功能角色名中的方位字）正则无法可靠完成——假阴性多（漏检）、假阳性也多（误报），白名单/黑名单无限膨胀。precheck 模块仅提取文本字段供 LLM 逐条审查，不做任何规则判定。
...
│   │   ├── direction_generalizer.py  # 方向泛化预处理 (绝对方向→功能描述)
...
5. **15 条规则全部 LLM 语义审查**：废止正则预检——中文文本中的"步骤1"与"3米"、方向"一侧"与数量"三名"、绝对方向"西端"与角色"侧翼警戒手"只有语义判断能可靠区分
