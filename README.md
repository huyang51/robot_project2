...
| **场景无关** | 战术描述行动**模式**，不绑定特定实例，禁止具体数值与品牌名 |
| **相对关系** | 空间用"紧邻/中距离"，时间用"即时/短暂延迟"，火力用"短点射/持续压制" |
...
```
  ...
  │    M3 迭代精炼 (最多 3 轮):                                                 │
  │         • A_gen: 生成/修正通用化双版本战术 JSON                               │
  │         • Python 正则预检: 14 条规则 (G-T1~G-T9, G-S1~G-S5)                │
  │         • A_review: 语义层通用性审查 + 军事可行性审查                        │
  │         • 硬约束违规 → A_gen 修正; 通过 → 军事审查                           │
  │         • 收敛检测: 连续两轮评分提升 < 0.5                                   │
  │    M4 评估:                                                                │
...
| P2 | `global_understanding.json` | LLM 子场景划分 + 人工审核 | `sub_scene_definitions.json` | 1 |
| P3 | P0 + P2 输出 | 裁剪 → 简化 → LLM 语义标注 → 校验 | `SS_XX/scene_cubes.json` + `desc.json` | N (每子场景 1 次) |
| P4 | P3 输出 | M1 验证 → M2 策略 → M3 迭代精炼 → M4 评估 | `tactics/*/{H,M,L}/*.json` | 3~5 per 子场景 |

---

## 三、Phase 4 详细流程

Phase 4 是整个流水线的核心，细分为四个子模块：
...
desc.json + scene_cubes.json
   ...
   │   └────────┘    修正指令反馈       └──────────┘      │
   │                                                       │
   │   全部 14 条规则由 A_review LLM 语义审查（无正则预检）  │
...
### M3 通用性审查规则 (14 条，全部由 A_review LLM 语义审查)

**文字描述版 (G-T1~G-T9)**：检查 text_version 的 Description / Objective / Action_Sequence Intent / Visual_Aids
...
| G-T7 | 软 | 禁止场景特定物体特征 | 颜色/品牌/型号/特定材质；通用掩体类型除外 |
...
> **设计决策**：全部 14 条规则由 A_review LLM 做语义审查。此前尝试用 Python 正则做前置预检，但中文文本的语义判断（区分步骤编号与参数绑定、区分方向用字与数量用字）正则无法可靠完成——假阴性多（漏检）、假阳性也多（误报），白名单/黑名单无限膨胀。precheck 模块仅提取文本字段供 LLM 逐条审查，不做任何规则判定。
...
# Phase 4: 战术生成（单个子场景，3-5 次 LLM 调用）
...
# Phase 4: 每子场景 3-5 次 LLM 调用；传入嵌入客户端启用 RAG
...
for ss in ss_results:
    ...
    print(f"{ss['sub_scene_id']}: {result['quality_level']}")
```
...
│   │   ├── schemas.py           # Tactic, ReviewFeedback, EvalResult
│   │   ├── m1_validator.py      # desc.json 一致性 + 战术标注
│   │   ├── m2_strategy.py       # 双层自适应策略 (RAG/Hybrid/GEN)
│   │   ├── m3_refinement/       # M3 迭代精炼
│   │   │   ├── agen_prompts.py       # A_gen 三模式变体
│   │   │   ├── areview_prompts.py    # A_review 审查 Prompt
│   │   │   ├── aeval_prompts.py      # 精简 A_eval (迭代控制用)
│   │   │   ├── few_shot_examples.py  # 5 条 Few-Shot 对比示例
│   │   │   ├── precheck.py           # 审查上下文提取（已废止正则预检）
...
5. **14 条规则全部 LLM 语义审查**：废止正则预检——中文文本中的"步骤1"与"3米"、方向"一侧"与数量"三名"只有语义判断能可靠区分
6. **从 robot_project 适配而非复制**：保留核心逻辑，清理遗留引用
