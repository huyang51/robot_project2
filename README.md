...
| 原则 | 说明 |
|------|------|
| **编组化描述** | 执行主体用功能性编组（"突击组"、"侦察组"），以"组"为后缀，不绑定机器人编号。禁止"节点""单元""手""员"后缀 |
| **双版本输出** | 文字描述版（VLA 理解）+ 结构化描述版（可执行指令），内容一致，形式不同 |
| **参数占位** | 结构化版中的坐标/速度/射速留为 `{占位符}`，执行阶段填入 |
| **场景无关（三重不变性）** | 战术描述行动**模式**，必须具有三重不变性——旋转不变性（场景旋转任意角度后每个字仍正确）+ 物体功能不变性（掩体替换为功能等价物体后每个字仍正确）+ 编队规模不变性（编队人数任意增减后编组交互描述仍正确）。禁止具体数值、禁止绝对方向指代（东/南/西/北+侧/端/段/翼/面/墙/角/缘/头/区/部/方/向），禁止场景特定物体身份描述（桌子/沙发/椅子/花坛/空调外机等），必须替换为战术功能类别（低矮掩体/大型遮挡物/柱状掩体等），禁止编队规模限定（双人/两人/三人/1名/2名/3单元/双组等），战术描述编组交互模式非兵力组成 |
...
```
  ...
  │         • A_review: 19 条通用性规则 (G-T1~G-T12 + G-S1~G-S6 + G-PHASE) + 军事可行性语义审查   │
  │         • 硬约束违规 → A_gen 修正; 通过 → 收敛/输出                          │
  │    M4 评估:                                                                │
  │         • A_eval 六维度评分 (每维度 Checklist 驱动)                          │
  │         • 否决条件: 军事可行/场景适配/粒度合规 < 3.0 → L 级不入库            │
  │         • 分级: H (Q≥8.0 且军事≥7.0 且粒度≥7.0) / M (6.0≤Q<8.0) / L       │
...
desc.json + scene_cubes.json
        ...
        ▼
   ┌──────────────────┐
   │ 系统枚举消毒       │  desc.json 英文 zone type 枚举 → 中文 (corridor→走廊, stairwell→楼梯井)
   │ sanitize_desc      │  纯翻译层，不涉及方向假设——通用化由 A_gen 提示词十重不变性框架驱动
   └────────┬─────────┘
        │
        ▼
   ┌─────────┐
   ...
   │   └────────┘    修正指令反馈       └──────────┘      │
   │                                                       │
   │   全部 19 条规则由 A_review LLM 语义审查（无正则预检）  │
...
### M3 通用性审查规则 (19 条，全部由 A_review LLM 语义审查)

**文字描述版 (G-T1~G-T12)**：检查 text_version 的 Description / Objective / Action_Sequence Intent / Visual_Aids / Semantic_Tags

| 规则 | 约束级 | 内容 |
|------|--------|------|
| G-T1 | 硬 | 禁止阿拉伯数字（步骤编号除外） |
| G-T2 | 硬 | 禁止中文数字（步骤编号除外）；"双""两""三"等数字词视为中文数字；"两翼包抄"不视为违规（战术概念） |
| G-T3 | 硬 | 禁止数值+量词后缀组合（台/个/名/辆/架/组/队/挺/支/具/枚/门/发/次/轮/波） |
| G-T4 | 硬 | 禁止具体距离数值+单位（米/m/公里/km/厘米/cm） |
| G-T5 | 硬 | 禁止具体时间数值+单位（秒/s/分钟/min/小时/h） |
| G-T6 | 硬 | 禁止具体射击数量（数值+发/弹/匣/梭） |
| G-T7 | 硬 | 禁止场景特定物体身份描述（桌子/沙发/椅子/木质/金属/混凝土→战术功能类别） |
| G-T8 | 软 | 禁止具体机器人编号（执行主体必须用功能编组名） |
| G-T9 | 硬 | 执行主体使用功能编组名，编组名须使用"组"后缀（突击组/压制组/侦察组/侧翼警戒组/断后掩护组/护卫组/留守组），禁止"节点""单元""手""员"后缀 |
| G-T10 | 硬 | 禁止绝对方向指代（东/南/西/北+侧/端/段/翼/面/墙/角/缘/头/区/部/方/向）；方向作为编组名一部分时除外（"侧翼警戒组"） |
| G-T11 | 硬 | 禁止英文词汇（战术文本全文为中文，任何英文词汇均视为违规） |
| G-T12 | 硬 | 禁止个体级称谓（队员/操作手/射手/领队/尖兵等，执行主体必须使用编组名，集体行动用"编队"） |

**结构化描述版 (G-S1~G-S6)**：检查 struct_version 的 Action_Sequence Instructions

| 规则 | 约束级 | 内容 |
|------|--------|------|
| G-S1 | 硬 | 目标位置使用 `{target_position}` 占位符格式 |
| G-S2 | 硬 | 距离参数使用 `{distance}` 占位符格式 |
| G-S3 | 硬 | 时间参数使用 `{delay}` 占位符格式 |
| G-S4 | 硬 | 射击数量使用 `{fire_count}` 占位符格式 |
| G-S5 | 硬 | 执行主体使用 `[Unit编组名]` 格式 |
| G-S6 | 硬 | 编组名须使用"组"后缀，禁止"节点""单元""手""员"等后缀 |

**阶段一致性 (G-PHASE)**：

| 规则 | 约束级 | 内容 |
|------|--------|------|
| G-PHASE | 硬 | Mission_Phase 字段值与指定阶段一致；动作链最终目的与该阶段核心意图一致：侦察=获取情报、进攻=夺取地形/歼灭、防御=固守地形、撤退=安全转移兵力 |

> **设计决策**：全部 19 条规则由 A_review LLM 做语义审查。此前尝试用 Python 正则做前置预检，但中文文本的语义判断（区分步骤编号与参数绑定、区分方向用字与数量用字、区分绝对方向词与功能编组名中的方位字）正则无法可靠完成——假阴性多（漏检）、假阳性也多（误报），白名单/黑名单无限膨胀。precheck 模块仅提取文本字段供 LLM 逐条审查，不做任何规则判定。
...
│   │   ├── direction_generalizer.py  # desc.json 系统枚举消毒 (英文 zone type → 中文翻译)
...
│   │   │   ├── few_shot_examples.py  # 9 条 Few-Shot 对比示例
│   │   │   ├── precheck.py           # 审查上下文提取
...
| `FEW_SHOT_CONFIG` | Few-Shot 示例注入策略 (9 条固定对比示例) |
...
5. **19 条规则全部 LLM 语义审查**：废止正则预检——中文文本中的"步骤1"与"3米"、方向"一侧"与数量"三名"、绝对方向"西端"与编组名"侧翼警戒组"、英文"corridor"与中文"走廊"只有语义判断能可靠区分
6. **三重不变性哲学**：从"禁止什么"升级为"理解为什么"——旋转不变性（空间关系层面）+ 物体功能不变性（物体描述层面）+ 编队规模不变性（编组描述层面），多层防线（穷举通用性红线 + A_gen 提示词教学 + A_review 19 条硬约束审查 + M4 粒度合规门槛）确保战术跨场景、跨朝向、跨物体身份、跨编队规模迁移
