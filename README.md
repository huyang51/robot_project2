...
| **场景无关（三重不变性）** | 战术描述行动**模式**，必须具有三重不变性——旋转不变性（场景旋转任意角度后每个字仍正确）+ 物体功能不变性（掩体替换为功能等价物体后每个字仍正确）+ 编队规模不变性（编队人数任意增减后角色交互描述仍正确）。禁止具体数值、禁止绝对方向指代（东/南/西/北+侧/端/段/翼/面/墙/角/缘/头/区/部/方/向），禁止场景特定物体身份描述（桌子/沙发/椅子/花坛/空调外机等），必须替换为战术功能类别（低矮掩体/大型遮挡物/柱状掩体等），禁止编队规模限定（双人/两人/三人/1名/2名/3单元/双组等），战术描述角色交互模式非兵力组成 |
...
desc.json + scene_cubes.json
        ...
        ▼
   ┌──────────────────┐
   │ 三重泛化预处理     │  desc.json 绝对方向 → 功能/关系描述
   │ direction_generalizer│  + 物体身份 → 战术功能类别 + 编队规模限定 → 编队/角色描述（安全网）
   └────────┬─────────┘
        │
        ▼
   ┌─────────┐
...
| G-T2 | 硬 | 禁止中文数字 | 区分方向/顺序用字（"一侧"、"下一"）与数量用字（"三名"、"两翼"、"双人"） |
...
│   │   ├── direction_generalizer.py  # 三重泛化预处理 (绝对方向→功能描述 + 物体身份→战术功能类别 + 编队规模→编队/角色描述)
...
│   │   │   ├── few_shot_examples.py  # 7 条 Few-Shot 对比示例
│   │   │   ├── precheck.py           # 审查上下文提取
...
6. **三重不变性哲学**：从"禁止什么"升级为"理解为什么"——旋转不变性（空间关系层面）+ 物体功能不变性（物体描述层面）+ 编队规模不变性（角色描述层面），三层防线（预处理安全网 + A_gen 提示词教学 + A_review 硬约束审查）确保战术跨场景、跨朝向、跨物体身份、跨编队规模迁移
